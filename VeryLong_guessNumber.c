...
int main(int argc, char *argv[]) {
  ...
  scanf("%s", name); 
                   /* Note: for string (and only for string),       */
  ...
  answer = rand() % MAX; /* rand() gives a large random integer number */
                         /* and "% MAX" cuts that                      */
                         /* number down to the range from 0 to 9 (why?)*/
  ...
  while (guess != answer) { /* the next 3 statements will be repeated   */
                            /*  whenever "guess" differ from "answer"	*/
    printf("Make a guess = ");
    scanf("%d", &guess);
    count = count + 1;
  }
